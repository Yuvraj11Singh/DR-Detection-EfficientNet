...
// ═══ CLOCK ═══
function updateClock() { const el=document.getElementById('liveClock'); if(el) el.textContent=new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false}); }
...
function showToast(msg,type='info',ms=3500) {
  ...
  t.innerHTML=`<div class="toast-dot"></div><span>${msg}</span>`; c.appendChild(t);
...
// ═══ SIDEBAR RESIZER ═══
const resizer=document.getElementById('sidebarResizer'); let isResizing=false;
resizer.addEventListener('mousedown',()=>{ isResizing=true; document.body.classList.add('resizing'); resizer.classList.add('active'); });
document.addEventListener('mousemove',e=>{ if(!isResizing) return; let w=Math.min(Math.max(e.clientX,200),500); document.documentElement.style.setProperty('--sidebar-width',w+'px'); });
...
document.addEventListener('keydown',e=>{
  const tag=e.target.tagName; if(['INPUT','TEXTAREA','SELECT'].includes(tag)||e.target.contentEditable==='true') return;
...
function switchView(id) {
  ...
  const nav=document.querySelector(`.sidebar-nav li[data-target="${id}"]`); if(nav) nav.classList.add('active');
...
function animateCount(el,target,suffix,dec) {
  if(!el) return; let s=0;
  const step=ts=>{ if(!s) s=ts; const p=Math.min((ts-s)/1800,1), e=1-Math.pow(1-p,3); el.textContent=(dec?(e*target).toFixed(dec):Math.round(e*target).toLocaleString())+(suffix||''); if(p<1) requestAnimationFrame(step); };
  requestAnimationFrame(step);
}
const statsObs=new IntersectionObserver(entries=>{ entries.forEach(e=>{ if(e.isIntersecting){ animateCount(document.getElementById('cnt-auc'),0.94,'',2); animateCount(document.getElementById('cnt-acc'),92,'%',0); animateCount(document.getElementById('cnt-params'),5.3,'M',1); statsObs.disconnect(); } }); },{threshold:0.5});
const ss=document.querySelector('.stats-strip'); if(ss) statsObs.observe(ss);
...
// ═══════════════════════════════════════════════
const CV={};
CV.load=(src,w=200,h=200)=>new Promise(resolve=>{ const img=new Image(); img.onload=()=>{ const c=document.createElement('canvas'); c.width=w; c.height=h; const ctx=c.getContext('2d'); ctx.drawImage(img,0,0,w,h); resolve({canvas:c,ctx,data:ctx.getImageData(0,0,w,h),img}); }; img.src=src; });
...
CV.sobel=src=>{ const w=src.width,h=src.height,g=CV.gray(src),d=new ImageData(new Uint8ClampedArray(src.data.length),w,h),gx=[-1,0,1,-2,0,2,-1,0,1],gy=[-1,-2,-1,0,0,0,1,2,1]; for(let y=1;y<h-1;y++) for(let x=1;x<w-1;x++){let sx=0,sy=0; for(let ky=-1;ky<=1;ky++) for(let kx=-1;kx<=1;kx++){const p=g.data[((y+ky)*w+(x+kx))*4],k=(ky+1)*3+(kx+1); sx+=gx[k]*p; sy+=gy[k]*p;} const m=Math.min(255,Math.sqrt(sx*sx+sy*sy)),i=(y*w+x)*4; d.data[i]=d.data[i+1]=d.data[i+2]=m; d.data[i+3]=255;} return d; };
CV.clahe=(src,tile=32,clip=3.0)=>{ const w=src.width,h=src.height,g=CV.gray(src),d=new ImageData(new Uint8ClampedArray(src.data.length),w,h),nx=Math.ceil(w/tile),ny=Math.ceil(h/tile),luts=[]; for(let ty=0;ty<ny;ty++){luts[ty]=[]; for(let tx=0;tx<nx;tx++){const hist=new Array(256).fill(0),x0=tx*tile,y0=ty*tile,x1=Math.min(x0+tile,w),y1=Math.min(y0+tile,h),n=(x1-x0)*(y1-y0); for(let y=y0;y<y1;y++) for(let x=x0;x<x1;x++) hist[g.data[(y*w+x)*4]]++; const cl=Math.floor(clip*n/256); let ex=0; for(let i=0;i<256;i++){if(hist[i]>cl){ex+=hist[i]-cl;hist[i]=cl;}} const pb=Math.floor(ex/256); for(let i=0;i<256;i++) hist[i]+=pb; const lut=new Uint8Array(256); let cdf=0,cmin=-1; for(let i=0;i<256;i++){cdf+=hist[i]; if(cmin<0&&cdf>0) cmin=cdf; lut[i]=Math.round((cdf-cmin)/(n-cmin)*255);} luts[ty][tx]=lut;}} for(let y=0;y<h;y++) for(let x=0;x<w;x++){const tx=(x/tile)-0.5,ty=(y/tile)-0.5,tx0=Math.max(0,Math.floor(tx)),ty0=Math.max(0,Math.floor(ty)),tx1=Math.min(nx-1,tx0+1),ty1=Math.min(ny-1,ty0+1),fx=tx-Math.floor(tx),fy=ty-Math.floor(ty),pv=g.data[(y*w+x)*4],v00=luts[ty0][tx0][pv],v10=luts[ty0][tx1][pv],v01=luts[ty1][tx0][pv],v11=luts[ty1][tx1][pv],val=v00*(1-fx)*(1-fy)+v10*fx*(1-fy)+v01*(1-fx)*fy+v11*fx*fy,sc=val/(pv+1e-6),i=(y*w+x)*4; d.data[i]=Math.min(255,src.data[i]*sc); d.data[i+1]=Math.min(255,src.data[i+1]*sc); d.data[i+2]=Math.min(255,src.data[i+2]*sc); d.data[i+3]=255;} return d; };
...
async function runAnalysis(src,cv,cvSize){
  previewImg.src=src; previewWrap.style.display='block'; previewImg.style.display='none';
  loadingOverlay.style.display='block'; resultCard.style.display='none';
  ...
  setTimeout(()=>{ const img=document.getElementById('preview-img'),ann=document.getElementById('annotationCanvas'); ann.width=img.offsetWidth; ann.height=img.offsetHeight; annHistory=[]; },100);
  ...
  updateSessionCounts(); updateSidebar(patient.uniqueKey); renderDashboard(patient); updateRegistryView();
...
function renderDashboard(patient){
  ...
  if(patient.filterUsed&&patient.filterUsed!=='none'&&patient.cv){ pp.style.display='block'; document.getElementById('processedLabel').textContent=patient.filterUsed.toUpperCase(); const map={clahe:'clahe',green:'green',edge:'edge'},key=map[patient.filterUsed]; if(key&&patient.cv[key]) CV.draw(document.getElementById('processedCanvas'),patient.cv[key],patient.cvSize,patient.cvSize); }
  ...
  const badge=document.getElementById('result-badge'); badge.textContent=patient.stageLevel===0?'Negative':'Refer to Ophthalmologist'; badge.className='result-badge '+patient.color;
  ...
  setTimeout(()=>{ document.getElementById('cert-ring').style.strokeDashoffset=264-264*(patient.certainty/100); document.getElementById('lesion-ring').style.strokeDashoffset=264-264*(Math.min(patient.density,100)/100); },200);
...
function toggleAnnotationMode(){
  ...
  toolbar.style.display=annotationMode?'flex':'none'; canvas.style.pointerEvents=annotationMode?'auto':'none';
...
const annCanvas=document.getElementById('annotationCanvas'),annCtx=annCanvas.getContext('2d'); let annSnapshot=null;
...
annCanvas.addEventListener('mousedown',e=>{ if(!annotationMode) return; isDrawing=true; const pos=getAnnPos(e); annStart=pos; annSnapshot=annCtx.getImageData(0,0,annCanvas.width,annCanvas.height); if(annTool==='pen'){annCtx.beginPath();annCtx.moveTo(pos.x,pos.y);} if(annTool==='text'){const txt=prompt('Enter label:'); if(txt){annCtx.font=`${+document.getElementById('annSize').value*4+10}px 'DM Mono'`; annCtx.fillStyle=document.getElementById('annColor').value; annCtx.fillText(txt,pos.x,pos.y); saveAnnSnapshot();} isDrawing=false;} });
annCanvas.addEventListener('mousemove',e=>{ if(!annotationMode||!isDrawing) return; const pos=getAnnPos(e),color=document.getElementById('annColor').value,size=+document.getElementById('annSize').value; if(annTool==='pen'){annCtx.strokeStyle=color;annCtx.lineWidth=size;annCtx.lineCap='round';annCtx.lineTo(pos.x,pos.y);annCtx.stroke();} else{annCtx.putImageData(annSnapshot,0,0);annCtx.strokeStyle=color;annCtx.lineWidth=size; if(annTool==='rect'){annCtx.beginPath();annCtx.strokeRect(annStart.x,annStart.y,pos.x-annStart.x,pos.y-annStart.y);} if(annTool==='circle'){annCtx.beginPath();const rx=(pos.x-annStart.x)/2,ry=(pos.y-annStart.y)/2;annCtx.ellipse(annStart.x+rx,annStart.y+ry,Math.abs(rx),Math.abs(ry),0,0,Math.PI*2);annCtx.stroke();} if(annTool==='arrow'){const dx=pos.x-annStart.x,dy=pos.y-annStart.y,angle=Math.atan2(dy,dx),hl=14;annCtx.beginPath();annCtx.moveTo(annStart.x,annStart.y);annCtx.lineTo(pos.x,pos.y);annCtx.stroke();annCtx.beginPath();annCtx.moveTo(pos.x,pos.y);annCtx.lineTo(pos.x-hl*Math.cos(angle-0.4),pos.y-hl*Math.sin(angle-0.4));annCtx.moveTo(pos.x,pos.y);annCtx.lineTo(pos.x-hl*Math.cos(angle+0.4),pos.y-hl*Math.sin(angle+0.4));annCtx.stroke();}} });
...
function downloadAnnotated(){ const merged=document.createElement('canvas'),img=document.getElementById('preview-img'); merged.width=img.naturalWidth; merged.height=img.naturalHeight; const mctx=merged.getContext('2d'); mctx.drawImage(img,0,0); mctx.drawImage(annCanvas,0,0,merged.width,merged.height); const a=document.createElement('a');a.download=`annotated_${Date.now()}.png`;a.href=merged.toDataURL('image/png');a.click(); showToast('Annotated image downloaded.','success'); }
...
function updateSidebar(activeKey){
  ...
  sessionHistory.forEach(pat=>{ const li=document.createElement('li'); if(pat.uniqueKey===activeKey) li.classList.add('active'); const dc=pat.stageLevel===0?'success':pat.stageLevel<=2?'warning':'danger'; li.innerHTML=`<div class="pat-list-info"><span class="status-dot ${dc}"></span><img src="${pat.imageSrc}" class="sidebar-thumb" alt=""/><div class="pat-list-details"><strong>${pat.id}</strong><span>${pat.stageName}</span></div></div><span class="pat-list-time">${pat.date}</span>`; li.addEventListener('click',()=>{switchView('detect');updateSidebar(pat.uniqueKey);renderDashboard(pat);}); list.appendChild(li); });
}

// ═══ QUICK NOTE ═══
function saveQuickNote(){ const text=document.getElementById('quickNoteInput').value.trim(); if(!text) return; const active=sessionHistory[0]; const note={id:Date.now().toString(),title:`Scan Note \u2014 ${active?.id||'Unknown'}`,content:text,tag:'finding',ts:new Date().toLocaleString()}; notes.unshift(note); document.getElementById('quickNoteInput').value=''; showToast('Note saved.','success'); renderNotesList(); }

// ═══ HEATMAP ═══
const toggleHeatmapBtn=document.getElementById('toggleHeatmapBtn'),xaiControls=document.getElementById('xai-controls'),heatmapOverlay=document.getElementById('heatmap-overlay'),heatmapSlider=document.getElementById('heatmapSlider');
toggleHeatmapBtn.addEventListener('click',()=>{ heatmapOverlay.classList.toggle('active'); const on=heatmapOverlay.classList.contains('active'); if(on){toggleHeatmapBtn.textContent='Disable Grad-CAM';toggleHeatmapBtn.style.cssText='background:rgba(0,221,180,0.12);color:var(--accent);border:1px solid rgba(0,221,180,0.4);font-family:"DM Mono",monospace;font-size:12px;padding:10px 20px;border-radius:8px;cursor:pointer;';xaiControls.style.display='block';heatmapOverlay.style.opacity=heatmapSlider.value/100;} else{toggleHeatmapBtn.textContent='Enable Grad-CAM';toggleHeatmapBtn.style.cssText='';xaiControls.style.display='none';heatmapOverlay.style.opacity=0;} });
...
// ═══ EXPORT SCAN JSON ═══
function exportScanJSON(){ if(!sessionHistory.length){showToast('No scan to export.','error');return;} const pat=sessionHistory[0],data={id:pat.id,age:pat.age,eye:pat.eye,hba1c:pat.hba1c,diab:pat.diab,date:pat.date,stageName:pat.stageName,stageLevel:pat.stageLevel,certainty:pat.certainty,density:pat.density,inference:pat.time,filter:pat.filterUsed,brightness:pat.brightness,contrast:pat.contrast}; const a=document.createElement('a');a.download=`retinaai_scan_${pat.id}.json`;a.href='data:application/json;charset=utf-8,'+encodeURIComponent(JSON.stringify(data,null,2));a.click(); showToast('Scan exported as JSON.','success'); }

// ═══ RESET ═══
function resetForm(){ document.getElementById('upload-flow').style.display='block'; document.getElementById('uploadArea').style.display='block'; document.getElementById('cv-preview-strip').style.display='none'; previewWrap.style.display='none'; resultCard.style.display='none'; fileInput.value=''; document.querySelectorAll('.patient-list li').forEach(li=>li.classList.remove('active')); heatmapOverlay.classList.remove('active'); xaiControls.style.display='none'; toggleHeatmapBtn.style.cssText=''; toggleHeatmapBtn.textContent='Enable Grad-CAM'; if(annotationMode) toggleAnnotationMode(); document.getElementById('severity-indicator').style.left='0%'; document.getElementById('cert-ring').style.strokeDashoffset=264; document.getElementById('lesion-ring').style.strokeDashoffset=264; ['pat-id','pat-age','pat-diab','pat-hba1c'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='';}); document.getElementById('processedPanel').style.display='none'; document.getElementById('detect').scrollIntoView({behavior:'smooth'}); }
...
function renderRegistryTable(){ const src=(document.getElementById('searchInput').value||document.getElementById('filterGrade').value)?filteredHistory:sessionHistory,data=[...src]; if(sortConfig.key) data.sort((a,b)=>{let va=a[sortConfig.key],vb=b[sortConfig.key];if(!isNaN(va))va=+va;if(!isNaN(vb))vb=+vb;return sortConfig.dir==='asc'?(va>vb?1:-1):(va<vb?1:-1);}); const tbody=document.getElementById('registryTableBody'),footer=document.getElementById('tableFooter'); tbody.innerHTML=''; if(!data.length){tbody.innerHTML='<tr><td colspan="9" style="text-align:center;color:var(--muted);padding:28px;">No records found.</td></tr>';footer.textContent='';return;} data.forEach(pat=>{const tr=document.createElement('tr'),bc=pat.stageLevel===0?'badge-negative':'badge-positive'; tr.innerHTML=`<td><strong>${pat.id}</strong></td><td>${pat.age}</td><td>${pat.eye}</td><td><span class="result-badge ${bc}" style="font-size:9px;padding:2px 9px;">${pat.stageName}</span></td><td style="font-family:'DM Mono',monospace;color:var(--accent);">${pat.certainty}%</td><td>${pat.hba1c}%</td><td style="font-family:'DM Mono',monospace;font-size:10px;color:var(--muted);">${pat.filterUsed||'none'}</td><td style="font-family:'DM Mono',monospace;font-size:10px;color:var(--muted);">${pat.date}</td><td><button onclick="event.stopPropagation();switchView('detect');updateSidebar('${pat.uniqueKey}');renderDashboard(sessionHistory.find(p=>p.uniqueKey==='${pat.uniqueKey}'))" style="font-family:'DM Mono',monospace;font-size:9px;padding:3px 9px;border-radius:5px;border:1px solid var(--border);background:transparent;color:var(--muted);cursor:pointer;">View</button></td>`; tr.addEventListener('click',()=>{switchView('detect');updateSidebar(pat.uniqueKey);renderDashboard(pat);}); tbody.appendChild(tr);}); footer.textContent=`Showing ${data.length} of ${sessionHistory.length} records`; }
...
function exportRegistryCSV(){ if(!sessionHistory.length){showToast('No data.','error');return;} const headers=['ID','Age','Eye','Diagnosis','Stage','Certainty','HbA1c','Diabetes','Filter','Brightness','Contrast','Time'],rows=sessionHistory.map(p=>[p.id,p.age,p.eye,p.stageName,p.stageLevel,p.certainty+'%',p.hba1c,p.diab,p.filterUsed,p.brightness,p.contrast,p.date]),csv=[headers,...rows].map(r=>r.join(',')).join('\n'); const a=document.createElement('a');a.download=`retinaai_registry_${Date.now()}.csv`;a.href='data:text/csv;charset=utf-8,'+encodeURIComponent(csv);a.click(); showToast('CSV exported.','success'); }

// ═══ IMAGE LAB ═══
function initLabUpload(){ const zone=document.getElementById('labUploadZone'),input=document.getElementById('labFileInput'); zone.onclick=()=>input.click(); zone.ondragover=e=>{e.preventDefault();zone.style.borderColor='var(--accent)';}; zone.ondragleave=()=>zone.style.borderColor=''; zone.ondrop=e=>{e.preventDefault();zone.style.borderColor='';const f=e.dataTransfer.files[0];if(f&&f.type.startsWith('image/'))loadLabImage(f);}; input.onchange=()=>{if(input.files[0])loadLabImage(input.files[0]);}; }
async function loadLabImage(file){ const reader=new FileReader(); reader.onload=async e=>{const {data}=await CV.load(e.target.result,400,300); currentLabImage=data; CV.draw(document.getElementById('labOrigCanvas'),data,400,300); document.getElementById('labUploadZone').style.display='none'; document.getElementById('labWorkspace').style.display='block'; currentLabFilter='original'; applyLabProcessing(); renderHistogram(data); renderLabStats(data); setupPixelInspector(); showToast('Image loaded.','info');}; reader.readAsDataURL(file); }
function getLabAdjusted(){ if(!currentLabImage) return null; return CV.adjust(currentLabImage,+document.getElementById('ctrl-brightness').value,+document.getElementById('ctrl-contrast').value,+document.getElementById('ctrl-saturation').value,+document.getElementById('ctrl-gamma').value/100,+document.getElementById('ctrl-sharpen').value); }
function applyLabProcessing(){ if(!currentLabImage) return; const adj=getLabAdjusted(),w=currentLabImage.width,h=currentLabImage.height; let out; switch(currentLabFilter){case 'grayscale':out=CV.gray(adj);break;case 'green':out=CV.channel(adj,1);break;case 'red':out=CV.channel(adj,0);break;case 'blue':out=CV.channel(adj,2);break;case 'clahe':out=CV.clahe(adj);break;case 'edge':out=CV.sobel(adj);break;case 'vessel':out=CV.vessel(adj);break;case 'pseudocolor':out=CV.pseudo(adj);break;case 'invert':out=CV.invert(adj);break;case 'emboss':out=CV.emboss(adj);break;case 'thermal':out=CV.thermal(adj);break;default:out=adj;} if(document.getElementById('compareModeToggle')?.checked){const ratio=+document.getElementById('compareSlider').value/100,blended=new ImageData(new Uint8ClampedArray(adj.data.length),w,h); for(let i=0;i<adj.data.length;i+=4){blended.data[i]=adj.data[i]*(1-ratio)+out.data[i]*ratio;blended.data[i+1]=adj.data[i+1]*(1-ratio)+out.data[i+1]*ratio;blended.data[i+2]=adj.data[i+2]*(1-ratio)+out.data[i+2]*ratio;blended.data[i+3]=255;} CV.draw(document.getElementById('labOutCanvas'),blended,w,h);}else{CV.draw(document.getElementById('labOutCanvas'),out,w,h);} document.getElementById('labOutputLabel').textContent=currentLabFilter.charAt(0).toUpperCase()+currentLabFilter.slice(1); }
...
['brightness','contrast','saturation','sharpen','gamma','blur'].forEach(id=>{ const ctrl=document.getElementById(`ctrl-${id}`),val=document.getElementById(`val-${id}`); if(!ctrl) return; ctrl.addEventListener('input',()=>{val.textContent=id==='gamma'?(ctrl.value/100).toFixed(2):ctrl.value; applyLabProcessing();}); });
...
function renderHistogram(data){
  ...
  histogramChart=new Chart(ctx,{
    ...
    data:{labels,datasets:[
      {label:'R',data:h.r,borderColor:'rgba(255,78,126,0.8)',backgroundColor:'rgba(255,78,126,0.1)',borderWidth:1,pointRadius:0,fill:true,tension:0.2},
      {label:'G',data:h.g,borderColor:'rgba(0,221,180,0.8)',backgroundColor:'rgba(0,221,180,0.1)',borderWidth:1,pointRadius:0,fill:true,tension:0.2},
      {label:'B',data:h.b,borderColor:'rgba(0,170,255,0.8)',backgroundColor:'rgba(0,170,255,0.1)',borderWidth:1,pointRadius:0,fill:true,tension:0.2}
    ]},
    options:{responsive:true,maintainAspectRatio:false,
      plugins:{legend:{labels:{color:'#6b8f9e',font:{family:'DM Mono,monospace',size:9},boxWidth:7}}},
      scales:{x:{ticks:{color:'#6b8f9e',font:{size:8}},grid:{color:'rgba(0,220,180,0.04)'}},y:{ticks:{color:'#6b8f9e',font:{size:8}},grid:{color:'rgba(0,220,180,0.04)'}}}
    }
  });
}
...
function computeRisk() {
  ...
  rc.textContent = category.label;
  rc.style.background = category.color+'22';
  rc.style.color = category.color;
  rc.style.border = `1px solid ${category.color}44`;
  ...
  // Breakdown list
  document.getElementById('riskBreakdownList').innerHTML = factors.filter(f=>f.score>0).map(f=>`
    <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border);">
      <div style="flex:1;">
        <div style="font-size:12px;color:var(--text);margin-bottom:3px;">${f.name}</div>
        <div style="height:3px;background:var(--surface);border-radius:100px;overflow:hidden;">
          <div style="width:${(f.score/f.max*100).toFixed(0)}%;height:100%;background:${f.color};border-radius:100px;transition:width 1s;"></div>
        </div>
      </div>
      <div style="font-family:'DM Mono',monospace;font-size:11px;color:${f.color};width:28px;text-align:right;">${f.score.toFixed(1)}</div>
    </div>`).join('') || '<div style="color:var(--muted);font-size:13px;padding:12px 0;">No significant risk factors identified.</div>';

  // Recommendations
  const recs = pct<25
    ? 'Annual fundus screening recommended. Maintain HbA1c below 7%, blood pressure below 130/80 mmHg, and BMI below 25. Continue current management plan.'
    : pct<50
    ? 'Biannual ophthalmology review advised. Intensify glycaemic control targeting HbA1c below 7%. Consider ACE inhibitor if microalbuminuria present. Lifestyle modification strongly recommended.'
    : pct<75
    ? 'Referral to ophthalmologist within 3 months. Urgent glycaemic optimization. Blood pressure target below 130/80. Rule out nephropathy. Consider cardiology co-management.'
    : 'URGENT ophthalmology referral within 4 weeks. Multidisciplinary management required. Aggressive glycaemic and BP control. Evaluate for end-organ damage. Consider anti-VEGF prophylaxis discussion.';
...
function exportRiskReport() {
  ...
  const hba1c = document.getElementById('rc-hba1c').value;
  const dur   = document.getElementById('rc-dur').value;
...
function saveAnalysisSettings() {
  appSettings.filter    = document.getElementById('set-filter').value;
  appSettings.threshold = +document.getElementById('set-threshold').value;
